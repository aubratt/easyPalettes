  ...
  _moveImage(e) {
    requestAnimationFrame(() => {
      if (this._img) {
        ...
        let left = Math.min(Math.max(pt.x - this._imgOffset.x, 0), w - this._img.offsetWidth);
        let top = Math.min(Math.max(pt.y - this._imgOffset.y, 0), h - this._img.offsetHeight);
